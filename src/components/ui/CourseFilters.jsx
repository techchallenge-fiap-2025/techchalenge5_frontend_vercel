...
const filterOptions = [
  ...
  { value: "antigo", label: "Último Adicionado" },
];

export function CourseFilters({ onFilterChange, onClose }) {
    ...
    const fetchMaterias = async () => {
      ...
      try {
        ...
        if (result.success && result.data) {
          const materiasOptions = result.data.map((materia) => ({
            value: materia._id,
            label: materia.nome,
          }));
  ...
  const handleClearFilters = () => {
    ...
    onFilterChange({ ordem: null, materiaId: null });
    setIsRotating(true);
    setTimeout(() => {
      setIsOpen(false);
      setIsRotating(false);
    ...
    <div className="relative" ref={filterRef}>
      ...
      <button
        onClick={() => {
          ...
          setTimeout(() => {
            setIsOpen(!isOpen);
            setIsRotating(false);
          }, 150); // Metade da duração da animação para trocar o ícone no meio
        }}
      ...
      {isOpen && (
        <div
          ...
          ref={filterRef}
        >
          <div className={`${isMobile ? "h-full flex flex-col" : ""} p-4 space-y-4`}>
            {/* Header */}
