...
const orderOptions = [
  ...
  { value: "antigo", label: "Menos Recente" },
];
...
export function StudentFilters({ onFilterChange }) {
    ...
    const fetchTurmas = async () => {
      ...
      try {
        ...
        if (result.success && result.data) {
          const turmasOptions = result.data.map((turma) => ({
            value: turma._id,
  ...
  const handleClearFilters = () => {
    ...
    onFilterChange({ ordem: null, turmaId: null, status: null });
    setIsRotating(true);
    setTimeout(() => {
      setIsOpen(false);
      setIsRotating(false);
        ...
        >
          <div className={`${isMobile ? "h-full flex flex-col overflow-y-auto" : ""} p-4 space-y-4`}>
            ...
            <div className={`flex gap-2 pt-2 ${isMobile ? "mt-auto" : ""}`}>
              ...
              {(selectedOrder || selectedTurma || selectedStatus) && (
                <button
