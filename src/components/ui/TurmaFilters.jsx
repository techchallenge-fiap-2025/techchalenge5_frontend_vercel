...
// Gerar opções de anos letivos (de 2000 até 10 anos à frente do ano atual)
const generateAnoLetivoOptions = () => {
...
export function TurmaFilters({ onFilterChange }) {
  const [isOpen, setIsOpen] = useState(false);
  const [selectedOrder, setSelectedOrder] = useState(null);
  const [selectedStatus, setSelectedStatus] = useState(null);
  const [selectedNivelEducacional, setSelectedNivelEducacional] = useState(null);
  const [selectedAnoLetivo, setSelectedAnoLetivo] = useState(null);
  const [selectedPeriodo, setSelectedPeriodo] = useState(null);
  const filterRef = useRef(null);
  const [isRotating, setIsRotating] = useState(false);
  const [anoLetivoOptions] = useState(generateAnoLetivoOptions());
  ...
  const handleNivelEducacionalChange = (selectedOption) => {
    setSelectedNivelEducacional(selectedOption);
  };

  const handleAnoLetivoChange = (selectedOption) => {
    setSelectedAnoLetivo(selectedOption);
  };
  ...
  const handleApplyFilters = () => {
    const filters = {
      ...
      nivelEducacional: selectedNivelEducacional?.value || null,
      anoLetivo: selectedAnoLetivo?.value || null,
      periodo: selectedPeriodo?.value || null,
    };
  ...
  const handleClearFilters = () => {
    ...
    setSelectedNivelEducacional(null);
    setSelectedAnoLetivo(null);
    setSelectedPeriodo(null);
    onFilterChange({ ordem: null, status: null, nivelEducacional: null, anoLetivo: null, periodo: null });
    setIsRotating(true);
    setTimeout(() => {
      setIsOpen(false);
      setIsRotating(false);
        ...
        <div
          className={`${
            isMobile
              ...
              : "absolute right-0 mt-2 w-80 bg-white border-2 border-orange-500 rounded-lg shadow-lg z-50 filter-dropdown-open max-h-[80vh] overflow-y-auto"
          }`}
          ref={filterRef}
        >
          <div className={`${isMobile ? "h-full flex flex-col overflow-y-auto" : ""} p-4 space-y-4`}>
            ...
            <CustomSelect
              label="Filtrar por Nível Educacional"
              options={nivelEducacionalOptions}
              value={selectedNivelEducacional}
              onChange={handleNivelEducacionalChange}
            ...
            <div className={`flex gap-2 pt-2 ${isMobile ? "mt-auto" : ""}`}>
              ...
              {(selectedOrder || selectedStatus || selectedNivelEducacional || selectedAnoLetivo || selectedPeriodo) && (
                <button
