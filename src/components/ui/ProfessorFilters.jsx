...
const orderOptions = [
  ...
  { value: "antigo", label: "Adicionado Antigamente" },
];
...
export function ProfessorFilters({ onFilterChange }) {
  ...
  // Buscar turmas e matérias do banco de dados
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [turmasResult, materiasResult] = await Promise.all([
          turmaService.getAll(),
          materiaService.getAll(),
        ]);

        if (turmasResult.success && turmasResult.data) {
          const turmasOptions = turmasResult.data.map((turma) => ({
            value: turma._id,
            label: turma.anoLetivo
              ? `${turma.nome} (${turma.anoLetivo})`
              : turma.nome,
          }));
          setTurmas(turmasOptions);
        }

        if (materiasResult.success && materiasResult.data) {
          const materiasOptions = materiasResult.data.map((materia) => ({
            value: materia._id,
            label: materia.nome,
          }));
  ...
  const handleApplyFilters = () => {
    const filters = {
      ...
      turmaId: selectedTurma?.value || null,
      materiaId: selectedMateria?.value || null,
  ...
  const handleClearFilters = () => {
    ...
    onFilterChange({ ordem: null, turmaId: null, materiaId: null, status: null });
    setIsRotating(true);
    setTimeout(() => {
      setIsOpen(false);
      setIsRotating(false);
        ...
        >
          <div className={`${isMobile ? "h-full flex flex-col overflow-y-auto" : ""} p-4 space-y-4`}>
            ...
            <div className={`flex gap-2 pt-2 ${isMobile ? "mt-auto" : ""}`}>
              ...
              {(selectedOrder || selectedTurma || selectedMateria || selectedStatus) && (
                <button
